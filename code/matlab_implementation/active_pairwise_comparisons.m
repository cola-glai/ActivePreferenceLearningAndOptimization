% do active pairwise comparisons
function [x,y]=active_pairwise_comparisons(p_active,y_active,X,w_star)
    R=0;
    ...
    while jj < n
        ...
        while sum(goodInds)>0
            ...
            [w,fval,flag,accuracy_p]=linear_program_solver([ p_active;X; H(:,i,j)' ],[y_active;Y; 1],0);
            % add by gylai
            % assume that the label for Qh(i,j) is -1
            [w,fval,flag,accuracy_n]=linear_program_solver([ p_active;X; H(:,i,j)' ],[y_active;Y; -1],0);
    ...
    %                 end
                    ...
                    [w,fval,flag,accuracy]=linear_program_solver([ p_active;X; H(:, i, j)' ],[ y_active;Y; Qh(i, j) ],0);
